...
public class ContaDAO implements Map<String, Conta> {
    ...
    public Conta put(String key, Conta c) {
        ...
        try (Connection conn = DriverManager.getConnection(DAOconfig.URL, DAOconfig.USERNAME, DAOconfig.PASSWORD);
             ...
             */
            // Actualizar a Conta
            stm.executeUpdate("INSERT INTO contas VALUES ('" + c.getId() + "', '" + c.getScore() + "', '"
                    + c.getVersao(c.getId) + "') ");
             ...
             * newAl.removeAll(oldAl); // Alunos que entram na turma, em relação ao que está
             * na BD
             * oldAl.removeAll(t.getAlunos()); // Alunos que saem da turma, em relação ao
             * que está na BD
             * try (PreparedStatement pstm =
             * conn.prepareStatement("UPDATE alunos SET Turma=? WHERE Num=?")) {
             * // Remover os que saem da turma (colocar a NULL a coluna que diz qual a turma
             * dos alunos)
             ...
             * pstm.executeUpdate();
             * }
             * // Adicionar os que entram na turma (colocar o Id da turma na coluna Turma da
             * tabela alunos)
